...
const generatorRepository = process.env.KOIPOND_GENERATOR_REPOSITORY
const generatorSha = process.env.KOIPOND_GENERATOR_SHA
...
const generator = parsePondGenerator({ repository: generatorRepository, sha: generatorSha })
if (!generator) {
  console.error(
    'Cannot prove the generator version. Reference pillowtalk-Qy/koipond with a full 40-character commit SHA.',
  )
  process.exit(1)
}
...
const preparedState = preparePondState(grid, user, user, previousState, generator)
...
console.log(
  `${stateFile}  pond revision ${nextState.revision}, ${nextState.fish.length} persistent fish, ` +
    `sha256:${nextState.proof.digest.slice(0, 12)}, generated by ${generator.repository}@${generator.sha}`,
)
